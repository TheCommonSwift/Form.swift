...
class Form {
    ...
    func setValue(value: String, forFieldWithIdentifier field: String,
        inSectionWithIdentifier section: String) throws {
        let s = try sectionWithIdentifier(identifier: section)
        let f = try s.fieldWithIdentifier(field)
        f.value = value
    }
    func valueForFieldWithIdentifier(field: String, inSectionWithIdentifier section: String)
        throws -> AnyObject? {
        let s = try sectionWithIdentifier(identifier: section)
        let f = try s.fieldWithIdentifier(field)
        return f.value
